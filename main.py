...
@app.route('/daily_price', methods=['POST']) 
def insert_data_daily_price():
    data = request.get_json()
    if not data:
        return jsonify({"error": "沒有提供資料"}), 400
...
@app.route('/get_t_0050_data', methods=['GET'])
def get_t_0050_data():
    # 檢查是否存在 't_0050' 表格
    table_name = 't_0050'
    metadata = MetaData(bind=db.engine)
    ...
    for row in result:
        records.append({
            ...
            "dividend_period": row.dividend_period,
            "shareholders_meeting_date": row.shareholders_meeting_date,
            "ex_dividend_date": row.ex_dividend_date,
            "ex_dividend_reference_price": row.ex_dividend_reference_price,
            "fill_dividend_completion_date": row.fill_dividend_completion_date,
            "fill_dividend_days": row.fill_dividend_days,
            "cash_dividend_distribution_date": row.cash_dividend_distribution_date,
            "ex_rights_date": row.ex_rights_date,
            "ex_rights_reference_price": row.ex_rights_reference_price,
            "fill_rights_completion_date": row.fill_rights_completion_date,
            "fill_rights_days": row.fill_rights_days,
            "cash_dividend_earnings": row.cash_dividend_earnings,
            "cash_dividend_capital_surplus": row.cash_dividend_capital_surplus,
            "total_cash_dividend": row.total_cash_dividend,
            "stock_dividend_earnings": row.stock_dividend_earnings,
            "stock_dividend_capital_surplus": row.stock_dividend_capital_surplus,
            "total_stock_dividend": row.total_stock_dividend,
